...
var ar_bid = (typeof(bid) == 'undefined') ? 0 : bid;
...
function ar_object(comppath, flash, link, w, h, gif){
	...
	if (!gif){
		...
		var ar_flashvars='link1=' + escape(link) + '&target=_blank&ar_comppath=' + escape(comppath) + '&ar_bid=' + ar_bid + '&ar_ntype=' + ar_ntype;
...
function im_setVis(obj, v){ar_d.getElementById(obj+ar_bid).style.visibility = v}
function showSG(){
...
function ar_s(){
    var o1 = ar_d.getElementById("sg_small"+ar_bid);
	var o2 = ar_d.getElementById("sg_big"+ar_bid);
    if (o1&&o2) {
...
if (ar_flashver<=ar_flver()){
	...
	+ '<div id="sg_small' + ar_bid + '">'
	+ ar_object(ar_comppath, smallflashname, ar_link, ar_width_small, ar_height_small)
	+ '</div><div id="sg_big' + ar_bid + '" style="position:absolute;visibility:hidden;width:'+ar_px(ar_width_big)+';height:'+ar_px(ar_height_big)+';">'
